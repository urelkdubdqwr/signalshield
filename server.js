  ...
  if(req.method==='POST' && u.pathname==='/api/inspect'){
    ...
    try { const data=JSON.parse(body); if(typeof data.claim!=='string'||!data.claim.trim()) throw new Error('claim must be non-empty');
      ...
      if (urls.length) {
        const fetched = [];
        const errors = [];
        for (const sourceUrl of urls.slice(0, 5)) {
          try { fetched.push(await inspectUrl(sourceUrl)); }
          catch (error) { errors.push({ url: sourceUrl, error: error.message }); }
        }
        if (fetched.length) {
          result.source = fetched[0];
          result.evidence = buildEvidenceLedger(data.claim.trim(), fetched);
          result.comparison = fetched.length > 1 ? compareEvidence(data.claim.trim(), fetched) : null;
        }
